...
const jwksUrl = 'https://dev-goyzfckz5ungnfpo.us.auth0.com/.well-known/jwks.json';
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const signingKeys = response.data.keys.find((key) => key.kid === jwt.header.kid);
  if (!signingKeys) throw new Error(`Unable to find a signing key that matches ${jwt.header.kid}`);
  const cert = signingKeys.x5c[0];
